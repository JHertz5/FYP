...
To set up the layer, zero padding, ouput depth, stride and kernel size parameters are read. In order for the convolution paramters to be valid, these must be set such that $outputDepth \geqslant 0$, $zeroPadding \geqslant 0$, $stride \geqslant 1$ and $ kernelSize \geqslant 2$. Then the output height and width are calculated using Equation \ref{eq:ConvOutSize}.
\begin{equation}
...
The ARM CPU on the Zynq-7000 provides a processing system on the Zedboard enabling the embeddded system to run software. This software consists of an operating system and Application Software, i.e. software that has been designed and built specifically for this project.
