...
\begin{itemize}
\item Describe need for analysis of impact of SEUs
\end{itemize}
...
\textbf{Section \ref{sec:Background} - Background:} In order to guarantee that the reader has the contextual knowledge required to understand the project, this section will describe the Target Platform, High Level Synthesis, and CNNs, and will discuss FPGA implementations of CNNs and the difficulties of using FPGAs in satellites.\\
\textbf{Section \ref{sec:ProjSpec} - Project Specification:} This section will details the project requirements that should be met by the final system.\\
\textbf{Section \ref{sec:Design} - Design:} The decisions taken in the design process are described and explained in this section. The high level design is described, followed by important aspects of the low level design.\\
\textbf{Section \ref{sec:Imp} - Implementation:} This section will go into detail about how the system was implemented, aiming to provide the reader with an understanding of the development process.\\
...
\textbf{Section \ref{sec:Conclusion} - Conclusion and Future Work:} This section will summarise the results of the project, highlight areas for improvement and further work, and present concluding remarks.\\
...
\subsection{Matrix Multiplication Function}
\label{sec:Imp-gemm}
\vspace{-12pt}

\begin{itemize}
\item Describe implementation of \verb|gemm()| function
\end{itemize}
